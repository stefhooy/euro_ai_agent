...
from ui.config import (
    ACTIVITY_MAP, COUNTRY_COUNT_MAP, DIRECTNESS_MAP,
    MIN_DAILY, MIN_TRANSPORT, PACE_MAP, ROUTE_PRIORITY_MAP,
    STYLE_MAP, TRANSPORT_MODE_MAP,
)


def render_sidebar(reset_callback: Callable) -> Tuple[bool, dict]:
    """Render the sidebar and return (plan_button_clicked, preferences_dict).

    preferences_dict is only valid when plan_button_clicked is True and the
    user has passed all validation checks inside the plan-button block.
    """
    with st.sidebar:
        st.markdown(
        ...
        budget_input = st.number_input(
            "Total Budget (€)", min_value=500, max_value=20_000, value=2500, step=100
        )
        ...
        if budget_input < _min_budget * 0.55:
            st.error(
                ...
                "Either increase your budget or switch to a cheaper travel style."
            )
            ...
            st.warning(
                ...
                "The agent will try to replan, but options will be limited."
            )
        ...
        route_priority_input = st.selectbox(
            "Route Preference", options=list(ROUTE_PRIORITY_MAP.keys()), index=0
        )
        directness_input = st.selectbox(
            "Directness", options=list(DIRECTNESS_MAP.keys()), index=0
        )
        ...
        st.caption(
            "Estimates only. Always verify prices on Skyscanner, Booking.com, or Airbnb before booking."
        )
