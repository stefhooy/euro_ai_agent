...
def _recalculate_full_plan(
    ...
    """Recalculate all costs based on a modified trip plan."""
...
def replan(
    ...
    """Autonomously adjust a trip plan to fit within the user's budget.

    Tries a series of cost-saving strategies in order until the plan
    is no longer over budget.

    Args:
        trip_plan: The current, over-budget trip plan.
        budget_result: Budget calculator result showing the deficit.
        all_scored_cities: Full list of top-scored cities.
        preferences: The original user preferences.

    Returns:
        Dict with adjusted_trip_plan, new_budget_result, changes_made.
    """
    logger.warning(
        f"Trip is over budget by €{-budget_result['buffer']:.2f}. "
        "Initiating replanning."
    )
    ...
    if current_budget_result["is_over_budget"]:
        accom = current_plan.get("accommodation", {})
        accom_breakdown = accom.get("city_breakdown", {})
        if accom_breakdown:
            city_to_cut = max(
                accom_breakdown,
                key=lambda c: accom_breakdown[c]["total_cost"],
            )
            if current_plan["nights_per_city"][city_to_cut] > 1:
                ...
                current_plan, current_budget_result = _recalculate_full_plan(
                    current_plan, preferences
                )
                if not current_budget_result["is_over_budget"]:
                    return {
                        "adjusted_trip_plan": current_plan,
                        "new_budget_result": current_budget_result,
    ...
    if current_budget_result["is_over_budget"]:
        ...
        if accom_breakdown and len(current_plan["destinations"]) > 1:
            city_to_swap = max(
                accom_breakdown,
                key=lambda c: accom_breakdown[c]["nightly_cost"],
            )
            current_dest_names = set(current_plan["destinations"])
            replacement = next(
                (
                    c for c in all_scored_cities
                    if c["name"] not in current_dest_names
                ),
                None,
            )
            if replacement:
                idx = current_plan["destinations"].index(city_to_swap)
                current_plan["destinations"][idx] = replacement["name"]
                nights = current_plan["nights_per_city"].pop(city_to_swap)
                current_plan["nights_per_city"][replacement["name"]] = nights
                change = (
                    f"Swapped {city_to_swap} with a more affordable "
                    f"option: {replacement['name']}."
                )
    ...
    if current_budget_result["is_over_budget"]:
        ...
        if original_style in style_map:
            ...
            change = (
                f"Downgraded travel style from "
                f"'{original_style}' to '{new_style}'."
            )
    ...
    if current_budget_result["is_over_budget"]:
        ...
        changes_made.append(change)
        city_acts = current_plan["activities"]["city_activities"]
        for city, details in city_acts.items():
            if details["activities"]:
                expensive = max(
                    details["activities"],
                    key=lambda act: act["cost_eur"],
                )
                if expensive["cost_eur"] > 0:
                    details["activities"].remove(expensive)
        current_plan, current_budget_result = _recalculate_full_plan(
            current_plan, preferences
        )
