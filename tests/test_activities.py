...
def test_activities_minimum_three_per_city():
    """Even with no preference matches, return at least 3 activities."""
    result = get_activities(
        ["Barcelona"], ["adventure"], {"Barcelona": 3}
    )
...
def test_activities_unknown_city_returns_empty():
    result = get_activities(
        ["Atlantis"], ["history"], {"Atlantis": 2}
    )
    acts = result["city_activities"]["Atlantis"]["activities"]
    assert acts == []
